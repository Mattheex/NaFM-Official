...
class Classyfire(InMemoryDataset):
    def __init__(self, root, dataset_arg, transform=None, pre_transform=None):
        self.dataset_arg = dataset_arg

        super(Classyfire, self).__init__(root, transform, pre_transform)
        self.data, self.slices = torch.load(self.processed_paths[0])

    @property
    def raw_file_names(self):
        return ["massbank_output.csv"]

    @property
    def processed_file_names(self):
        return [f"Classyfire_{self.dataset_arg}.pt"]
    ...
    def process(self):
        for path, processed_path in zip(self.raw_paths, self.processed_paths):
            ...
            for i in trange(len(smiles_list)):
                ...
                if mol is None:
                    print(f"Error parsing SMILES at index {i}: {smiles_list[i]}")
                    continue
                mol = Chem.RemoveHs(mol)

                unsupported_atoms = [atom.GetAtomicNum() for atom in mol.GetAtoms() 
                             if atom.GetAtomicNum() not in ATOM_LIST]
        
                if unsupported_atoms:
                    # This skips the molecule if it contains Arsenic (33), Hydrogen (1), etc.
                    print(f"Skipping index {i} (contains {unsupported_atoms})")
                    continue
...
if __name__ == "__main__":
    dataset = Classyfire(
        root="../../downstream_data/ClassyFire", dataset_arg="superclass"
    )
